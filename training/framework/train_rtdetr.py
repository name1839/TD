...
def parse_args():
    ...
    parser.add_argument('--model', type=str, default='rtdetr-l.pt', help='model file or config file path')
    parser.add_argument('--epochs', type=int, default=200)
    parser.add_argument('--imgsz', type=int, default=640)
    parser.add_argument('--batch', type=int, default=16)
    parser.add_argument('--device', type=str, default='0')
    parser.add_argument('--workers', type=int, default=8)
    parser.add_argument('--name', type=str, default='rtdetr_l', help='save results folder name')
    parser.add_argument('--exist-ok', action='store_true', help='overwrite existing experiment folder')
    parser.add_argument('--patience', type=int, default=30, help='early stopping epochs')
    parser.add_argument('--test-only', action='store_true', help='only test')
    parser.add_argument('--cal-ivtd', type=str, default=None, help='Calculate IVT metrics from saved predictions directory (e.g., runs/rtdetr_l_test/labels)')
...
def test(model, args):
    if args.weights:
        if os.path.exists(args.weights):
            ...
            print(f"Warning: weights file {args.weights} not found, using loaded model")
    
    print("Testing RTDETR model...")
    ...
    try:
        ...
        print(f"Warning: failed to read class names: {e}")
...
def main():
    ...
    if args.apply_ivt_metrics:
        apply_ivt_metrics_patch(args.mapping_file)

    if args.tool_nms:
        apply_tool_nms_if_requested(args.mapping_file)
    ...
    if args.test_only:
        ...
        print_metrics(test_results, class_names)
        return
    ...
    print("RTDETR triplet classification training and testing completed successfully!")
