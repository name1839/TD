...
def high_quality_resize(img, target_size):
    """multi-step progressive resizing"""
...
def process_single_image(args):
    ...
    try:
        ...
        base_name = os.path.splitext(img_file)[0]  # get the file name without the extension
        ext = os.path.splitext(img_file)[1].lower()  # get the extension (.jpg or .png)
        ...
        with Image.open(img_path) as img:
            ...
            # create a square background image (depending on whether there is an Alpha channel)
            if has_alpha:
        ...
        # calculate the exact scale factor
        scale_factor = float(target_width) / float(square_size)  # 使用浮点数确保精确计算
        ...
        if os.path.exists(json_path):
            ...
            # save the complete transformation parameters, for precise restoration
            transform_info = {
            ...
            for shape in data["shapes"]:
                for i, point in enumerate(shape["points"]):
                    original_point = point.copy()  # save the original coordinates for verification
...
def restore_single_json(args):
    """restore single JSON annotation to the original size"""
    ...
    try:
        ...
        if "transformInfo" in data:
            ...
            with print_lock:
                print(f"警告：{json_file} 缺少还原所需的变换信息，无法还原")
            return False
        
        data["imageWidth"] = original_width
        data["imageHeight"] = original_height
        
        # restore the annotation coordinates (check if there are original coordinates)
        for shape in data["shapes"]:
            if "originalPoints" in shape:
                ...
                # otherwise, restore the coordinates by reverse calculation
                for i, point in enumerate(shape["points"]):
                    # reverse transformation: scale -> square -> original
...
def resize_image_and_annotation(input_dir, output_dir, target_size=(640, 640), num_workers=16):
    """
    multi-threaded resizing of images and updating the corresponding annotation files, keeping the original aspect ratio
    
    Args:
        input_dir: input directory path
        output_dir: output directory path
        target_size: target size, default (640, 640)
        num_workers: number of threads, default 16
    """
...
if __name__ == "__main__":
    ...
    resize_input_dir = "./prostate_track_v2"
    resize_output_dir = "./prostate_v2_640"
    target_size = (640, 640)
